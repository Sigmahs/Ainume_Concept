...
From scratch basic concept website in which logging in would allow the user access to a NLP based telehealth assistant.
...
```bash
conda create python=3.8 -n ainume
```
2 - <a name="step-2">Activate the conda environment</a>

```bash
conda activate ainume
```
...
If for some reason you get an error similar to bellow, is because the DATABASE_URL is configured to `postgres:///ainume` and because of it the generated `DATABASES` settings are configured to connect on PostgreSQL using the socket mode.
...
LINE 1: CREATE DATABASE ainume WITH ENCODING 'UTF-8' OWNER "";
                                                             ^
ERROR:  syntax error at or near ";"
LINE 1: GRANT ALL PRIVILEGES ON DATABASE ainume TO ;
```
...
ERROR:  database "ainume" already exists
GRANT
```
...
```bash
sudo -u postgres dropdb -U postgres --if-exists ainume
